...
//---------------------视频end---------------------
   ...
   */
    ...
    getShareInfo(type: number, picIndex: number) {
        return GameUtils.format("channel={1}&uid={2}&act={3}&picIndex={4}",WXUtils.InviteChannel.self,GameDataManager.getInstance().userLocalData.openid, type, picIndex + 10);
    }
    onShareResume(res){
        // if(this.shareCallBack){
    ...
    videoError(res){
        ...
        GameUtils.showTip("今日广告次数已看完！请明日再试。");
    ...
    videoSuccess(res){
        ...
        if (res && res.isEnded || res == undefined) {
            ...
            GameUtils.showTip("观看完整视频才能获得奖励哦！");
