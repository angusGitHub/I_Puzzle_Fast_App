...
def reducePic(srcFile,dstFile):
	cmd = "pngquant -f --quality 20-30 --output %s %s" %(srcFile, dstFile)
	os.system(cmd)
...
def yaShuo(gameMulu,mulu):
	...
	g_curDir = gameMulu + '/build/' + mulu + '/assets/'
	traverse(g_curDir)
...
def main():
	gameMulu = '/Users/lidongdong/Documents/SVN/Puzzle/client/proj/I_Puzzle_Fast_App'
	mulu = "I_Puzzle_Fast_App"
	...
	print('删除完成')
	cmd = "/Applications/CocosCreator/Creator/2.4.3/CocosCreator.app/Contents/MacOS/CocosCreator --path " + gameMulu + " --build"
	os.system(cmd)
	print('编译弯沉')

	source_path_1 = os.path.abspath(gameMulu + '/build/jsb-default/src')
	target_path_1 = os.path.abspath(gameMulu + '/build/'+mulu+'/src')
	source_path_2 = os.path.abspath(gameMulu + '/build/jsb-default/assets')
	target_path_2 = os.path.abspath(gameMulu + '/build/'+mulu+'/assets')
